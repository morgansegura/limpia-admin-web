...
import { Check, Clock, AlertCircle, Loader2, FileText, CreditCard, Calendar, Users, Bell, Wrench } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { EstimateWorkflow, WorkflowStep, useWorkflow } from '@/hooks/useWorkflow';
...
export function WorkflowProgress({ workflow, onStepAction }: WorkflowProgressProps) {
  const { user } = useAuth();
  const [selectedStep, setSelectedStep] = useState<WorkflowStep | null>(null);
  const [isDialogOpen, setIsDialogOpen] = useState(false);

  const canManageWorkflow = user?.role === UserRole.SALES_MANAGER || 
      ...
      case 'contract_signing':
        return (
          <div className="space-y-2">
            ...
            {step.data?.signatureUrl && (
              <Button
                variant="outline"
                onClick={() => window.open(step.data.signatureUrl, '_blank')}
                className="w-full"
              >
          ...
          <div className="space-y-2">
            ...
            {step.data?.paymentId && (
              <p className="text-sm text-gray-600">
                Payment ID: {step.data.paymentId}
              </p>
            )}
          </div>
        );
      ...
      case 'service_scheduling':
        return (
          <div className="space-y-2">
            ...
            {step.data?.scheduledDate && (
              <p className="text-sm text-gray-600">
                Scheduled: {new Date(step.data.scheduledDate).toLocaleDateString()}
              </p>
            )}
          </div>
        );
      ...
      case 'crew_assignment':
        return (
          <div className="space-y-2">
            ...
            {step.data?.crewId && (
              <p className="text-sm text-gray-600">
                Crew: {step.data.crewName || step.data.crewId}
              </p>
            )}
          </div>
        );
            ...
            {workflow.steps.map((step, index) => (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-lg border ${
                  index <= workflow.currentStep ? 'border-blue-200' : 'border-gray-200'
                ...
                <div className="flex-1">
                  ...
                  {step.data && step.status === 'completed' && (
                    <p className="text-sm text-gray-600 mt-1">
                      Completed: {new Date(step.data.completedAt || Date.now()).toLocaleDateString()}
                    </p>
                  )}
                </div>
              </div>
          ...
          {workflow.isActive && (
            <div className="mt-4 p-3 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-700">
                <strong>Next Step:</strong> {workflow.steps[workflow.currentStep]?.name || 'Workflow Complete'}
              </p>
            </div>
          )}
        </div>
