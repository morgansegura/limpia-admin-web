...
const COMMISSION_TIERS = [
  ...
  },
  {
    ...
    description: "Fair deal. Higher discount reduces commission but still profitable.",
  },
  {
    tier: "POOR",
    discountRange: "25%+",
    ...
    description: "Large discount significantly reduces commission. Consider justification.",
  },
];
...
export function IntegratedSalesWorkflow({ onClose, onComplete, initialCalculatorData }: IntegratedSalesWorkflowProps) {
  ...
  const pricingCalculation = useMemo(() => {
    ...
    if (initialCalculatorData && 
      ...
      const calculatorTier = COMMISSION_TIERS.find(t => t.tier === initialCalculatorData.commissionTier) || COMMISSION_TIERS[0];
    ...
    console.log("🔢 Using integrated workflow calculation (fallback or modified data)");
    
    if (!formData.squareFootage || !formData.serviceType || !formData.frequency) {
      return {
        basePrice: 0,
        quotedPrice: 0,
        discountAmount: 0,
    ...
    const serviceMultiplier = SERVICE_MULTIPLIERS[formData.serviceType as keyof typeof SERVICE_MULTIPLIERS]?.multiplier || 1;
    const frequencyMultiplier = FREQUENCY_MULTIPLIERS[formData.frequency as keyof typeof FREQUENCY_MULTIPLIERS]?.multiplier || 1;
  ...
  const canProceedToStep = useCallback((step: number) => {
    switch (step) {
      case 2:
        return formData.customerName && formData.customerEmail && formData.customerAddress;
      case 3:
        return formData.serviceType && formData.squareFootage && formData.frequency;
      case 4:
        return pricingCalculation.quotedPrice > 0 && pricingCalculation.isDiscountAllowed;
      default:
        return true;
    }
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return <WelcomeStep initialCalculatorData={initialCalculatorData} onStepSelect={setCurrentStep} />;
      case 2:
        return <CustomerInfoStep formData={formData} updateFormData={updateFormData} />;
      case 3:
        return (
          <ServiceDetailsStep 
            formData={formData} 
...
function WelcomeStep({ initialCalculatorData, onStepSelect }: WelcomeStepProps) {
  if (initialCalculatorData) {
    return (
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-6">
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
              We've imported your commission calculations. Now let's gather customer 
              information and service details to create a professional estimate.
            </p>
            
            {/* Show imported calculation summary */}
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 max-w-md mx-auto mb-6">
              <h3 className="font-medium text-green-800 mb-2">Imported Calculation</h3>
              <div className="text-sm space-y-1">
                <div className="flex justify-between">
                  <span>Base Price:</span>
                  <span className="font-medium">${initialCalculatorData.basePrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Final Price:</span>
                  <span className="font-medium">${initialCalculatorData.finalPrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{initialCalculatorData.squareFootage} sq ft</span>
                </div>
                <div className="flex justify-between">
                  <span>Frequency:</span>
                  <span className="font-medium">{initialCalculatorData.frequency}</span>
                </div>
                <div className="flex justify-between">
                  <span>Service:</span>
                  <span className="font-medium">
                    {SERVICE_MULTIPLIERS[initialCalculatorData.serviceType as keyof typeof SERVICE_MULTIPLIERS]?.name || initialCalculatorData.serviceType}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Tier:</span>
                  <Badge className={COMMISSION_TIERS.find(t => t.tier === initialCalculatorData.commissionTier)?.color}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
              ...
              >
                ...
                <div className="mt-2 text-xs text-blue-600 font-medium">Click to jump to step →</div>
              </button>
              ...
              >
                ...
                <div className="mt-2 text-xs text-green-600 font-medium">Click to jump to step →</div>
              </button>
            </div>
          </div>
        </CardContent>
      ...
      <CardContent className="space-y-6">
        <div className="text-center py-8">
          <h2 className="text-xl font-semibold mb-4">
            Create Professional Estimates with Integrated Commission Calculations
          </h2>
          <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
            This workflow guides you through creating accurate estimates while automatically
            calculating your commission based on discount levels. Everything is integrated
            to help you close deals faster and maximize earnings.
          </p>
...
function ServiceDetailsStep({ formData, updateFormData, handlePetChange }: ServiceDetailsStepProps) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => updateFormData({ isRealtor: checked })}
              />
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => updateFormData({ isRushJob: checked })}
              />
...
interface PricingCommissionStepProps {
  ...
  initialCalculatorData?: any;
}

function PricingCommissionStep({ formData, updateFormData, pricingCalculation, initialCalculatorData }: PricingCommissionStepProps) {
  return (
        ...
        <Card className="border-green-200 bg-green-50">
          <CardContent className="pt-4">
            <div className="flex items-center space-x-2 text-green-700">
              ...
              <span className="font-medium">Using Pre-Calculated Data from Quick Calculator</span>
            </div>
            <p className="text-sm text-green-600 mt-1">
              Your pricing and commission calculations have been imported. You can adjust the discount if needed.
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            ...
            <span>
              {initialCalculatorData ? "Pricing Review & Adjustment" : "Live Commission Calculator"}
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {formData.discountPercentage > 0 && (
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => updateFormData({ discountReason: e.target.value })}
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium">${pricingCalculation.quotedPrice.toFixed(2)}</span>
              </div>
              
              {pricingCalculation.discountAmount > 0 && (
                <div className="flex justify-between text-red-600">
                  <span>Discount:</span>
                  <span>-${pricingCalculation.discountAmount.toFixed(2)}</span>
                </div>
              )}
              ...
              <div className="flex justify-between">
                <span>Commission ({pricingCalculation.commissionTier.commissionRate}%):</span>
                <span className="font-medium text-green-600">
                  ${pricingCalculation.commissionAmount.toFixed(2)}
                </span>
              </div>
            </div>
          </div>
          ...
          {!pricingCalculation.isDiscountAllowed && (
            <div className="flex items-start space-x-2 p-3 bg-red-50 border border-red-200 rounded-lg">
              ...
              <div>
                <div className="font-medium text-red-800">Discount Exceeds Maximum</div>
                <div className="text-sm text-red-600">
                  This discount reduces the margin below the 30% minimum threshold.
                  Minimum allowed price: ${pricingCalculation.minimumPrice.toFixed(2)}
                </div>
              </div>
            </div>
          )}
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">{tier.discountRange}</div>
                  </div>
                </div>
...
function ReviewFinalizeStep({ formData, pricingCalculation, updateFormData, onSubmit, isLoading }: ReviewFinalizeStepProps) {
  return (
    <div className="space-y-6">
      {/* Summary Cards */}
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-1">
              ...
              <div className="text-sm text-muted-foreground">{formData.customerEmail}</div>
              <div className="text-sm text-muted-foreground">{formData.customerPhone}</div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-1">
              <div className="font-medium">
                {SERVICE_MULTIPLIERS[formData.serviceType as keyof typeof SERVICE_MULTIPLIERS]?.name}
              </div>
              <div className="text-sm text-muted-foreground">
                {formData.squareFootage} sq ft • {FREQUENCY_MULTIPLIERS[formData.frequency as keyof typeof FREQUENCY_MULTIPLIERS]?.name}
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-1">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-muted-foreground">
                  {pricingCalculation.commissionTier.commissionRate}%
                </span>
              </div>
            </div>
          </CardContent>
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            <div className="flex justify-between text-green-600">
              ...
              <span className="font-semibold">${pricingCalculation.commissionAmount.toFixed(2)}</span>
            </div>
          </div>
        </CardContent>
      </Card>
