...
import { Phone, Clock, User, FileText, Save, X } from 'lucide-react';
...
const CALL_OUTCOMES = [
  { value: 'connected', label: 'Connected - Spoke with customer', color: 'bg-green-500' },
  ...
  { value: 'disconnected', label: 'Disconnected/Invalid number', color: 'bg-red-500' },
  { value: 'callback_requested', label: 'Customer requested callback', color: 'bg-blue-500' },
];
...
export function PhoneCallModal({ isOpen, onClose, customer, onCallLogged }: PhoneCallModalProps) {
  const { toast } = useToast();
  const [callState, setCallState] = useState<'ready' | 'calling' | 'connected' | 'completed'>('ready');
                ...
                <div className="text-right">
                  ...
                  {callState === 'connected' && callStartTime && (
                    <div className="text-sm text-gray-600 mt-1">
                      Duration: {formatDuration(Math.floor((Date.now() - callStartTime.getTime()) / 1000))}
                    </div>
                  )}
                </div>
              </div>
          ...
          {callState === 'ready' && (
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={startCall} className="w-full" disabled={!formData.purpose}>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-medium">Calling {customer.name}...</h3>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="text-lg font-medium">{formatDuration(callDuration)}</div>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFormData(prev => ({ ...prev, customerMood: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
